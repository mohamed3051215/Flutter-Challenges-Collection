...
const Map<ColorType, Color> COLOR_MAP = <ColorType , Color>{
...
class ShapePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Paint borderPaint = Paint()
      ..color = COLOR_MAP[color] ?? Colors.red
      ..strokeWidth = 3.0
  ...
  Paint getInteriorPaint(Size size) {
    switch (texture) {
      case TextureType.filled:
        return Paint()..color = COLOR_MAP[color] ?? Colors.red;
  ...
  List<Color> getColors(int numberOfBands) {
    return List.generate(
            ...
            (index) => [
                  Colors.transparent,
                  Colors.transparent,
                  COLOR_MAP[color] ?? Colors.red,
                  COLOR_MAP[color] ?? Colors.red
              ]).expand<Color>(( e) => e).toList() +
...
class SetHomeState extends State<SetHome> {
  late List<List<SetCard>> cardGrid;
  @override
  void initState() {
    super.initState();
